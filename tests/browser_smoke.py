...
with sync_playwright() as p:
    ...
    page.wait_for_function('document.getAnimations().filter(a=>a.playState==="running").length===0',timeout=7000)
    check('finite entry animations end',page.evaluate('document.getAnimations().filter(a=>a.playState==="running").length')==0)
